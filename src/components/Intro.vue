<template>
  <div id="intro-container">
    <div id="splash">
      <div id="speed" class="logo">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 160.056 23.097">
          <g transform="translate(0 0)">
            <path
              class="letters"
              d="M-1226.829,212.509a8.648,8.648,0,0,1-5.858-1.838,6.332,6.332,0,0,1-2.121-5.031,6.493,6.493,0,0,1,.956-3.478,7.75,7.75,0,0,1,2.84-2.613,4.477,4.477,0,0,1-1.607-1.765,5.4,5.4,0,0,1-.555-2.489,5.109,5.109,0,0,1,1.789-4.236,8.234,8.234,0,0,1,5.254-1.441h25.753v22.891Zm-1.836-18.941a2.407,2.407,0,0,0-.873,2.033,2.45,2.45,0,0,0,.873,2.047,4.094,4.094,0,0,0,2.588.689h22.065v-5.451h-22.016A4.244,4.244,0,0,0-1228.665,193.568Zm-1.474,9.114a3.634,3.634,0,0,0-1.035,2.761,3.7,3.7,0,0,0,1.035,2.8,4,4,0,0,0,2.907,1h23.22V201.7h-23.22A4.038,4.038,0,0,0-1230.139,202.682Z"
              transform="translate(1234.808 -189.496)"
              fill="#ffd400"
            />
            <g transform="translate(39.29 0.589)" id="e-top" class="e-burger">
              <path
                d="M-1195.518,192h28.113"
                transform="translate(1195.518 -190.085)"
                fill="#ffd400"
              />
              <path
                d="M-1188.8,193.867c.006-.075-.09-.147.059-.182.448-.069.819.175,1.273.1l.035-.119,1.094.174,2.07-.084c-.019-.046-.028-.083.038-.1a10.751,10.751,0,0,1,1.09.022l.182-.017.064-.094.5.028c2.011-.233,4.05-.507,6.261-.779a12.459,12.459,0,0,1,1.384-.271c.669-.044,1.518.033,2.169.048-.1-.421,1.483-.625,1.171-1.047.428.009,1.084.022,1.117-.1.1-.355.434-.765,1.586-.98.451-.051,1.142.1,1.3.028.25-.116-.589-.252-.337-.366-3.349-.129-6.569.026-9.945.024-1.147.186-8.889.268-10.88.186l-.18,0a19.294,19.294,0,0,0-3.321.169l-1.279.333.544.208a3.4,3.4,0,0,0-1.033.057c-.925.178-.4.389-.044.673.257.126-.094.317.31.446-.288-.007-.435-.009-.728-.018-.031.119.9-.057.693.134a6.633,6.633,0,0,0-.878.022c.1.162,1,.062.957.219-.16.035-.814-.215-.869-.019-.011.039.279.046.421.05-.888.059-1.377.248-.566.463a9.7,9.7,0,0,0-.978-.061h0c.149.008.41.05.544.052l-.507-.011.086.083.388.047-.336-.009.284.188a6.05,6.05,0,0,0,1,.318C-1192.307,193.786-1190.552,193.853-1188.8,193.867Z"
                transform="translate(1195.518 -190.085)"
                fill="#ffd400"
              />
            </g>
            <g transform="translate(39.166 9.566)" id="e-mid" class="e-burger">
              <path
                d="M-1195.518,200.723h28.113"
                transform="translate(1195.642 -199.062)"
                fill="#ffd400"
              />
              <path
                d="M-1176.3,199.652h.623c-3.514-.184-7.236-.111-10.543-.372-.211-.038-.621-.108-.829-.149a52.637,52.637,0,0,0-5.374,0,3.183,3.183,0,0,0-1.447.449c-.206.072-.41.224-.206.3.41.034,1.449.034,1.449.183,0,.037-.206.037-.416.037,0,.151-.623.45-.827.45-.206.074-1.446.294-1.652.37-.621.114,1.242.671,2.066.783.413.073,0,.186,0,.3.413-.037.623.075,1.033.036a17.075,17.075,0,0,0,1.862.074,5.98,5.98,0,0,1,.827-.035c.2,0,.2-.039.2-.075v.075a6.407,6.407,0,0,1,.831-.039v.074c.41-.074.821,0,1.237-.035v.075c.209,0,.413,0,.413.037v.074c.208.038.618-.037.618.038v-.075c.829,0,1.45.037,2.069.075a46.953,46.953,0,0,0,5.579,0c.209,0,.417-.075.209-.149h.622c.415,0,.207-.114.617-.114,3.312-.185,6.617,0,9.926-.11-.827-.038-1.449-.11-2.276-.075,0-.035,0-.114-.206-.114a26.559,26.559,0,0,1-3.306-.111,3.574,3.574,0,0,0-1.037-.148h.623v-.113c-.829-.034-1.862-.075-2.689-.11v-.112h-.41c-.413-.261,1.858-.224,2.893-.261v-.149h-.417v-.073h2.687c0-.037,0-.076-.206-.076.831,0,1.862.039,2.687-.035.208-.042,0-.152-.619-.189h-1.65v-.073c.413,0,1.033.036,1.444,0,.206-.037-.411-.113-.207-.147l.623-.041c-.623,0-1.447.041-2.072,0-.206-.037-.41-.075-.41-.149a7.618,7.618,0,0,1-2.483-.261A13.445,13.445,0,0,0-1176.3,199.652Z"
                transform="translate(1195.642 -199.062)"
                fill="#ffd400"
              />
            </g>
            <g
              transform="translate(39.29 18.87)"
              id="e-bottom"
              class="e-burger"
            >
              <path
                d="M-1195.518,210.171h28.113"
                transform="translate(1195.518 -208.366)"
                fill="#ffd400"
              />
              <path
                d="M-1194.59,208.89l.077.018-.215.007a8,8,0,0,0,2.7-.092c.263.008.491-.038.519-.1s-.168-.118-.436-.124l-.164-.022a9.179,9.179,0,0,0-1.339-.209l.292.055-.449-.033.054.036a3.869,3.869,0,0,0-1.541.063c-.349.118-.228.281.278.362Z"
                transform="translate(1195.518 -208.366)"
                fill="#ffd400"
              />
              <path
                d="M-1193.651,209.248l-.882-.127-.057-.022.018.1.006-.008a6.033,6.033,0,0,1,1.13-.136l.149-.009a.28.28,0,0,0,.131-.053c.037-.048-.09-.09-.285-.1l-.223-.009a4.193,4.193,0,0,0-1.687.107c-.07.037-.011.081.008.125-.023.062.181.117.452.125s.5-.042.518-.109l.14.013-.111.022a1.943,1.943,0,0,1,.506-.095l.274-.015-.155-.147-.139.018.025.015-.819.107-.379.055c-.1.029-.09.07.02.1l.166.033-.076-.022,1.166.09.165-.013c.026-.012,0-.032-.061-.038Z"
                transform="translate(1195.518 -208.366)"
                fill="#ffd400"
              />
              <g transform="translate(0 0.258)">
                <path
                  d="M-1195,209.264l-.209-.016-.037.011.116.026C-1195.042,209.3-1195,209.29-1195,209.264Z"
                  transform="translate(1195.518 -208.624)"
                  fill="#ffd400"
                />
                <path
                  d="M-1195.25,209.259l-.05-.011.033.015h.009Z"
                  transform="translate(1195.518 -208.624)"
                  fill="#ffd400"
                />
                <path
                  d="M-1178.535,208.989l3.56.066.076-.013-.066-.02-3.555-.066-.079.015Z"
                  transform="translate(1195.518 -208.624)"
                  fill="#ffd400"
                />
                <path
                  d="M-1169.664,209.9c-.028.007-.028.018,0,.022h.1l0-.022Z"
                  transform="translate(1195.518 -208.624)"
                  fill="#ffd400"
                />
                <path
                  d="M-1169.92,210.614l-.465.051h-.079c.009.015-.018.03-.081.033-1.18.123-2.448.175-3.7.241l-.051.033a1.236,1.236,0,0,1-.439.048l-.138-.009-1.642-.061-.384.033-1.424.121.047.037c-.01.031-.121.053-.255.053h-.193l.566.026.074.008.63,0c1.862-.032,3.612-.151,5.347-.287l.1,0-.013.022c-1.1.158-2.047.366-3.284.472-1.476.123-2.926.232-4.439.311-1.577,0-3.038.075-4.649.132-2.174.079-4.229.09-6.359.1a2.616,2.616,0,0,1-1.63-.294c-.061-.113.119-.219.45-.293l-.195,0-.311-.022a2.565,2.565,0,0,1-.7-.135l-.215-.077c-.285-.147-.752-.274-.985-.428l.053-.063-.057-.014a.645.645,0,0,1-.349-.16l.018-.074a.571.571,0,0,1,.278-.228l.049-.086c-.126-.074-.183-.151-.084-.235-.075-.022-.066-.052.029-.07l.1-.011h0l-.108-.044-.116-.011-.375-.022a1.548,1.548,0,0,1-.241-.063c-.163-.125-.163-.125-.139-.18l.033-.017c-.133-.02-.142-.022-.178-.025l-.061-.022-.03-.041-.005-.022.073-.033.045-.026a.859.859,0,0,1,.115-.013l.2.076.041-.026.042.033-.022-.031.1.02-.111-.029a2.82,2.82,0,0,0,.311.011,1.586,1.586,0,0,0,.171,0V209.3c.235.014.235.014.467.025l.055.023.11-.011.123,0,.024,0h0l.1-.017.02-.006h.006c.07-.007.169-.024.441-.062l.2.015-.057-.015c-.04-.009,0-.039-.023-.041-.506-.062-1.061-.086-1.6-.117-.094,0-.19-.013-.2-.039s.058-.04.156-.048a15.981,15.981,0,0,1,3.586-.007l.9.041-.046-.015.233.02.026,0-.178-.052-.122-.014c-.7-.022-1.329-.1-2-.151l-1.018-.114c-.088,0-.149-.022-.138-.044s.092-.033.178-.033a16.855,16.855,0,0,1,3.358.173l.574.047.013,0-.131-.042c.155.011.239.044.383.061l-.054-.059c.765.035,1.6.02,2.311.105l.082,0a7.862,7.862,0,0,1,.975-.084l-.361-.019.805-.038a7.113,7.113,0,0,1,2.024.248l4.443-.074c.077,0,.147.011.156.03s-.041.033-.116.037l-3.215.119,2.73.064.353-.005.351.011c1.176,0,2.3.005,3.5-.035,1.03-.039,1.927-.077,2.952-.132,1.44-.078,2.743-.155,4.165-.155.19-.005.364.026.386.07s-.117.088-.3.092c-2.961.14-5.869.307-8.966.285l-.987-.007.008.014c0,.026-.03.05-.085.066l1.125.03c1.038-.052,2.086-.1,3.2-.076.763.019,1.456.074,2.162.12l2.69-.088.087.011-.041.02-1.9.134c.1.031.162.064.153.105s-.083.077-.206.1l.437,0c.09,0,.16.015.156.037s-.071.037-.159.037l-1.78-.007-2.137.055a.462.462,0,0,1,.206.1h0l.334,0c2.013-.11,4.079-.176,6.156-.154l.079.018-.07.015-1.877,0h0c.024.009.031.016,0,.024h-.1c-.024,0-.024-.015,0-.022l-3.46.114,3.177,0,.347-.035.088.012-.048.021-.1,0c.2,0,.355.045.349.094s-.182.088-.391.088l-1.666.017-.178.009-.182.022,2.577.068c.075,0,.14.013.143.033s-.06.033-.132.033l-4.577.147-1.518.225.2,0c1.665,0,3.082-.213,4.7-.257l.081.013-.063.016-2.292.155-1.871.138c1.234-.021,2.414-.113,3.638-.153l.138.007.033-.011.483-.055.1.009C-1169.865,210.6-1169.887,210.61-1169.92,210.614Zm-21.42-1.028-.028-.005.107.014.147.074Zm-1.886.263h0c-.052-.026-.085-.04-.085-.04Zm12.039-.616-1.926-.06.468-.011Zm-11.42.088a3.188,3.188,0,0,1,.432.131l.344-.009.939.025c-.04-.023.033-.022.07-.023l.116,0-.987-.061Zm-2.273.1-.031-.007.064-.037Zm-.1.063.048.005Zm.428.049-.06-.022C-1194.553,209.529-1194.55,209.529-1194.55,209.529Zm.3,1.092.165-.186-.178.179Z"
                  transform="translate(1195.518 -208.624)"
                  fill="#ffd400"
                />
              </g>
            </g>
            <g transform="translate(74.293)" class="letters">
              <path
                d="M-1141.5,193.218c-.418-.2-.906-.419-1.455-.645a14.66,14.66,0,0,0-1.89-.612,22.027,22.027,0,0,0-2.389-.452,21.96,21.96,0,0,0-2.952-.182,23.879,23.879,0,0,0-3.49.226,9.973,9.973,0,0,0-2.594.7,4.13,4.13,0,0,0-1.621,1.2,2.716,2.716,0,0,0-.559,1.717,2.19,2.19,0,0,0,.528,1.461,4.771,4.771,0,0,0,1.428,1.112,13,13,0,0,0,2.108.875q1.211.392,2.564.739t2.794.7q1.434.355,2.79.768c.9.276,1.757.585,2.566.923a9.835,9.835,0,0,1,2.106,1.191,5.472,5.472,0,0,1,1.43,1.555,3.843,3.843,0,0,1,.528,2.025,4.768,4.768,0,0,1-.471,2.18,5.036,5.036,0,0,1-1.272,1.6,7.266,7.266,0,0,1-1.882,1.118,13.344,13.344,0,0,1-2.3.7,19.868,19.868,0,0,1-2.529.364c-.873.07-1.728.108-2.571.108a29.8,29.8,0,0,1-5.263-.428,30.411,30.411,0,0,1-4.5-1.167v-2.481a14.877,14.877,0,0,0,4.678,1.766,27.066,27.066,0,0,0,5.18.479,25.659,25.659,0,0,0,3.547-.227,11.589,11.589,0,0,0,2.814-.715,4.676,4.676,0,0,0,1.847-1.278,2.919,2.919,0,0,0,.658-1.93,2.583,2.583,0,0,0-.526-1.606,4.794,4.794,0,0,0-1.433-1.215,11.236,11.236,0,0,0-2.112-.925q-1.213-.4-2.562-.748t-2.787-.678q-1.441-.333-2.788-.72a26.326,26.326,0,0,1-2.562-.875,9.671,9.671,0,0,1-2.112-1.142,5.309,5.309,0,0,1-1.432-1.513,3.72,3.72,0,0,1-.528-1.994,4.462,4.462,0,0,1,.421-1.981,4.653,4.653,0,0,1,1.156-1.5,6.765,6.765,0,0,1,1.726-1.061,11.922,11.922,0,0,1,2.112-.682,19.252,19.252,0,0,1,2.336-.362q1.217-.112,2.4-.114c.864,0,1.684.039,2.452.114s1.516.175,2.235.3,1.419.268,2.105.428,1.378.335,2.078.519Z"
                transform="translate(1160.515 -189.496)"
                fill="#ffd400"
              />
              <path
                d="M-1133.365,212.178V189.91h2.066v22.268Z"
                transform="translate(1160.515 -189.496)"
                fill="#ffd400"
              />
              <path
                d="M-1099.492,201.026a14.677,14.677,0,0,1-.447,3.8,9.872,9.872,0,0,1-1.257,2.892,8.121,8.121,0,0,1-1.972,2.091,10.107,10.107,0,0,1-2.581,1.377,14.881,14.881,0,0,1-3.082.754,24.755,24.755,0,0,1-3.5.235h-11.5V189.91h11.473a22.907,22.907,0,0,1,5.091.535,10.7,10.7,0,0,1,4.084,1.823,8.748,8.748,0,0,1,2.708,3.426A12.761,12.761,0,0,1-1099.492,201.026Zm-2.068.01a13.13,13.13,0,0,0-.356-3.219,8.245,8.245,0,0,0-1.022-2.428,6.447,6.447,0,0,0-1.621-1.731,8.608,8.608,0,0,0-2.156-1.131,12.491,12.491,0,0,0-2.608-.61,24.211,24.211,0,0,0-3-.176h-9.438v18.606h9.438a20.4,20.4,0,0,0,4.36-.432,8.614,8.614,0,0,0,3.4-1.494,7.058,7.058,0,0,0,2.213-2.86A11.221,11.221,0,0,0-1101.56,201.036Z"
                transform="translate(1160.515 -189.496)"
                fill="#ffd400"
              />
              <path
                d="M-1093.535,212.178V189.91h18.548v1.831h-16.364v7.738h15.537v1.832h-15.537v9.036h16.6v1.831Z"
                transform="translate(1160.515 -189.496)"
                fill="#ffd400"
              />
            </g>
          </g>
        </svg>
      </div>
      <div id="text1" class="beside-tags">
        <p>
          Move fast. Speed is one of your main advantages over large
          competitors. - Sam Altman
        </p>
      </div>
      <div id="text2" class="beside-tags">
        <p>
          The primary factor in a successful attack is speed. - Lord Mountbatten
        </p>
      </div>
      <div id="text3" class="beside-tags">
        <p>
          There are no speed limits on the road to success. - David W. Johnson
        </p>
      </div>
      <div id="text4" class="beside-tags">
        <p>
          Speed has never killed anyone. Suddenly becoming stationary, that's
          what gets you. - Jeremy Clarkson
        </p>
      </div>
      <div id="text5" class="beside-tags">
        <h2>
          Speed defines everything.
        </h2>
      </div>
      <div id="get-in-touch">
        <h3>get in touch</h3>
      </div>
      <img id="down" src="../assets/icons/down-arrow.svg" alt="" />
    </div>
    <div class="news-inner" id="news-1">
      <div class="vert-tabs">
        <div
          v-for="newsArt in newsList"
          :key="newsArt.id"
          class="tabs"
          :id="newsArt.id"
          v-on:click="getNews(newsArt.id2)"
        >
          <p>{{ newsArt.shortMonth }}</p>
          <p>{{ newsArt.year }}</p>
        </div>
      </div>
      <div
        v-for="newsArt in newsList"
        :id="newsArt.id2"
        :key="newsArt.id"
        class="news-content"
      >
        <div class="img-container">
          <img class="news-img" :src="newsArt.imgSmall" />
          <video autoplay class="news-vid" controls>
            <source :src="newsArt.vidSmall" type="video/mp4" />
          </video>
        </div>
        <div class="news-txt">
          <h2>{{ newsArt.title }}</h2>
          <p>{{ newsArt.text }}</p>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
import { TweenMax, TimelineMax } from "gsap/TweenMax";
export default {
  data() {
    return {
      newsList: [
        {
          id: "news1",
          id2: "news1-content",
          shortMonth: "aug",
          year: "2019",
          title: "one-two punch...",
          text:
            "Come and visit our new Diesel and Fred Perry stores at Mall of Arabia, Cairo.",
          imgSmall: require("@/assets/img/news/small/MAC-opening-small.jpg"),
          vidSmall: ""
        },
        {
          id: "news2",
          id2: "news2-content",
          shortMonth: "jun",
          year: "2019",
          title: "two birds with one stone...",
          text:
            "New locations, new concepts and new collections. Now at the newly opened Fred Perry and Diesel stores in Dubai Festival City.",
          imgSmall: require("@/assets/img/news/small/DFC-opening-small.jpg"),
          vidSmall: ""
        },
        {
          id: "news3",
          id2: "news3-content",
          shortMonth: "may",
          year: "2019",
          title: "a helping hand...",
          text:
            "Ramadan is a time of giving and we at Beside Group wanted to help our community by distributing hundreds of iftar meals to blue collar workers. We would like to especially thank our partners Companies for Good for helping us organize this activity.",
          imgSmall: "#",
          vidSmall: require("@/assets/video/beside-iftar-sm.mp4")
        },
        {
          id: "news4",
          id2: "news4-content",
          shortMonth: "apr",
          year: "2019",
          title: "Grazia 2019",
          text:
            "We are delighted and proud to announce that Pinko has won the award for the “Contemporary Fashion Brand Of The Year” category in last night’s Grazia Style Awards. We would like to take this opportunity to thank all who voted and a special thanks goes to the Pinko team for being instrumental in winning this award. The team was represented by our colleagues Stephanie & Myncho who accepted the award on our behalf. While this prize was won by Pinko it is a source of pride for all of us at Beside Group as it, once again, demonstrates our high execution and commitment to excellence and we look forward to further success with all our brands.",
          imgSmall: require("@/assets/img/news/small/Grazia-awards-2019-sm.jpg"),
          vidSmall: ""
        },
        {
          id: "news5",
          id2: "news5-content",
          shortMonth: "mar",
          year: "2019",
          title: "just around the corner...",
          text:
            "Hello Dubai Marina!  Our newest Fred Perry shop is now open in Dubai Marina Mall.",
          imgSmall: require("@/assets/img/news/small/dmm-fp-sm.jpg"),
          vidSmall: ""
        }
      ]
    };
  },
  methods: {
    getNews(newsTitle) {
      var i, news, tabs, checkVid, checkImg, checker;
      news = document.getElementsByClassName("news-content");
      for (i = 0; i < news.length; i++) {
        news[i].style.display = "none";
      }
      tabs = document.getElementsByClassName("tabs");
      for (i = 0; i < tabs.length; i++) {
        tabs[i].className = tabs[i].className.replace(" active", "");
      }
      document.getElementById(newsTitle).style.display = "block";
      // event.currentTarget.className += " active";
      document.getElementById(newsTitle.replace("-content", "")).className +=
        " active";

      checkVid = document.getElementsByClassName("news-vid");
      checkImg = document.getElementsByClassName("news-img");

      for (i = 0; i < checkVid.length; i++) {
        // console.log(checkVid[i].currentSrc);

        if (checkVid[i].currentSrc === "") {
          checkVid[i].style.display = "none";
          checkImg[i].style.display = "block";
        } else {
          checkVid[i].style.display = "block";
          checkImg[i].style.display = "none";
        }
      }
    }
  },
  mounted() {
    this.getNews("news1-content");
    // document.getElementById("news1").click();

    const tl = new TimelineMax();
    const speedE = document.getElementById("speed"),
      letters = document.querySelectorAll(".letters"),
      audio = document.createElement("audio");

    // console.log(audio);
    audio.loop = false;
    audio.src = require("@/assets/wav/woosh-01.wav");
    audio.type = "audio/wav";
    audio.volume = 0.8;

    TweenMax.set(letters, { opacity: 0 });
    TweenMax.set(".beside-tags", { opacity: 0 });
    TweenMax.set("#get-in-touch", { opacity: 0 });

    tl.fromTo(
      speedE,
      1.5,
      {
        scaleX: 100,
        x: 1800,
        transformOrigin: "0% 100%",
        ease: Power4.easeOut
      },
      {
        scaleX: 1,
        x: -1800,
        transformOrigin: "0% 100%",
        ease: Power4.easeOut
      }
    );
    tl.call(audio.play(), 3);
    tl.to("#text2", 1, { opacity: 1 });
    tl.to("#text2", 3, { opacity: 0 });
    tl.fromTo(
      speedE,
      1.5,
      {
        scaleX: 100,
        x: 1800,
        transformOrigin: "0% 100%",
        ease: Power4.easeOut
      },
      {
        scaleX: 1,
        x: -1800,
        transformOrigin: "0% 100%",
        ease: Power4.easeOut
      }
    );
    tl.to("#text3", 1, { opacity: 1 });
    tl.to("#text3", 3, { opacity: 0 });
    tl.fromTo(
      speedE,
      1.5,
      {
        scaleX: 100,
        x: 1800,
        transformOrigin: "0% 100%",
        ease: Power4.easeOut
      },
      {
        scaleX: 1,
        x: -1800,
        transformOrigin: "0% 100%",
        ease: Power4.easeOut
      }
    );
    tl.to("#text4", 1, { opacity: 1 });
    tl.to("#text4", 3, { opacity: 0 });
    tl.fromTo(
      speedE,
      1.5,
      {
        scaleX: 100,
        x: 1800,
        transformOrigin: "0% 100%",
        ease: Power4.easeOut
      },
      {
        scaleX: 1,
        x: -1800,
        transformOrigin: "0% 100%",
        ease: Power4.easeOut
      }
    );
    tl.to("#text5", 1, { opacity: 1 });
    tl.to("#text5", 3, { opacity: 0 });
    tl.fromTo(
      speedE,
      1.5,
      {
        scaleX: 100,
        x: 1800,
        transformOrigin: "0% 100%",
        ease: Power4.easeOut
      },
      {
        scaleX: 1,
        x: 0,
        transformOrigin: "0% 100%",
        ease: Power4.easeOut
      }
    );

    tl.to(letters, 1, { opacity: 1, ease: Power1.easeOut, delay: 0.5 });
    tl.to("#get-in-touch", 1, { opacity: 1, ease: Power1.easeOut, delay: 0.5 });

    TweenMax.fromTo(
      "#down",
      1,
      { y: -10, ease: Bounce.easeOut, repeat: -1, yoyo: true },
      { y: 0, ease: Bounce.easeOut, repeat: -1, yoyo: true }
    );
  }
};
</script>

<style lang="scss" scoped>
#intro-container {
  margin-top:-50px;
  display: grid;
  grid-template-columns: repeat(10, 10vw);
  grid-template-rows: repeat(2, 100vh);

  #splash {
    width: 100%;
    display: grid;
    align-items: center;
    justify-items: center;
    grid-column: 1 / 11;
    grid-row: 1 / 2;
    grid-template-columns: repeat(10, 1fr);
    grid-template-rows: repeat(5, 20vh);
    background-color: blue;

    #speed {
      grid-column: 3 / 9;
      grid-row: 1 / 2;

      svg {
        width: 200px;
      }
    }

    .beside-tags {
      grid-column: 3 / 9;
      grid-row: 1 / 2;

      p {
        text-align: center;
      }
    }

    #get-in-touch {
      grid-column: 3 / 9;
      grid-row: 2 / 3;
      border: 3px solid #ffd400;
      padding: 10px;
    }

    #down {
      grid-column: 5 / 7;
      grid-row: 3 / 4;
      width: 30px;
      align-self: end;
    }
  }

  #news-1 {
    min-width: 100vw;
    min-height: 450px;
    grid-column: 1 / 10;
    grid-row: 2 / 3;
    background-color: lightblue;
    display: grid;

    .img-container {
      width: 100%;
      height: 450px;

      img {
        max-height: 100%;
        max-width: 100%;
        object-fit: contain;
      }
    }
  }
}
</style>
